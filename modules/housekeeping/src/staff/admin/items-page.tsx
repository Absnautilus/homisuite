...
function CategoryRow({ category, jobTitles, onToggle, onRemove, onSaved }: { category: RequestCategoryAdmin; jobTitles: JobTitleOption[]; onToggle: () => void; onRemove: () => void; onSaved: () => Promise<void> }) {
  const { t } = useLocale(); const [open, setOpen] = useState(false); const [pickerOpen, setPickerOpen] = useState(false); const [mansioniOpen, setMansioniOpen] = useState(false); const mansioniTriggerRef = useRef<HTMLButtonElement>(null)
