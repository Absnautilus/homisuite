...
-- now also resolves the hotel's contact details from the Core property's
-- settings jsonb via legacy_property_mapping, staying null (not an error)
-- when there's no mapping or the fields were never filled in -- the guest
-- UI must never fall back to some other, wrong contact.
begin;
...
-- Hotel Con Contatti is bridged to a Core property with phone+address set.
...
insert into properties (id, organization_id, name, slug, settings) values
  ('00000049-0000-0000-0000-000000000011', '00000049-0000-0000-0000-000000000010', 'Contact Test Property', 'test-049-prop',
   '{"phone": "+39 02 1234567", "address": "Via Roma 1, Milano"}'::jsonb);
...
select is(
  (select hotel_address from guest_stay_info(:'token_unmapped')),
  null,
  'an unmapped hotel resolves a null address too'
);
